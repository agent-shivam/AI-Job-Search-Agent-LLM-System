...
def search_jobs_node(state: AgentState) -> dict:
    ...
    if not query or not location:
        print("[WARN] Missing query/location — skipping search")
        return {"jobs_found": [], "jobs_scored": []}

    try:
        result = search_jobs(state)
        print(f"[SEARCH] Found {len(result.get('jobs_scored', []))} scored jobs")
        return result
...
def pick_job_node(state: AgentState) -> dict:
    ...
    if not jobs:
        print("[WARN] No jobs found — ending pipeline")
    ...
    for job in jobs:
        ...
        if job_id not in skipped:
            print(f"[PICK] Picked: {job.get('title')} @ {job.get('company')} (id={job_id})")
            return {"current_job": job}

    print("[WARN] All jobs exhausted (all skipped)")
    return {"current_job": None}
...
# 🚨 BLOCK BAD JD
    if len(jd_text) < 300 or jd_text.count(" ") < 80:
        print("[BAD JD] Bad JD detected → skipping LLM call")
...
def score_job_node(state: AgentState) -> dict:
    ...
    if not jd_data:
        print("[WARN] No parsed JD → using raw job data")
...
"""
    ...
    if score <= 0.5:
        score = 0.6
    print(f"[SCORE] score_job_node: {score:.2f} for {job.get('title', '?')} @ {job.get('company', '?')}")
...
def _load_base_resume() -> dict:
    ...
    # Resolve relative to project root (one level up from agent/ directory)
    if not os.path.isabs(resume_path):
        root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        resolved_path = os.path.join(root_dir, resume_path)
    else:
        resolved_path = resume_path
    ...
    candidates = [
        resolved_path,
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "data", "base_resume.json"),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "base_resume.json"),
