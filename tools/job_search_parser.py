...
def search_jsearch(query: str, location: str, date_posted: str = "month", num_pages: int = 2) -> List[dict]:
    """
    ...
    # Map today/3days/week/month
    api_date_posted = date_posted
    if api_date_posted == "all":
        api_date_posted = "all"
    ...
    for page in range(1, num_pages + 1):
        try:
            ...
            for item in data:
                ...
                # Capture extra details returned by JSearch
                job_required_experience = item.get("job_required_experience", {})
                req_exp = job_required_experience.get("no_of_years_required") if isinstance(job_required_experience, dict) else None
                jobs.append({
                    ...
                    "parsed_experience_years": req_exp,
                })
    ...
    print(f"[DATA] JSearch: {len(jobs)} jobs")
    return jobs


# ── Strategy 2: SerpAPI Google Jobs ──────────────────────────────────────────

def search_serpapi(query: str, location: str, date_posted: str = "month") -> List[dict]:
    """
    ...
    chip_val = date_posted
    chips = f"date_posted:{chip_val}" if chip_val != "all" else ""
    ...
    print(f"[DATA] SerpAPI: {len(jobs)} jobs")
    return jobs
...
def search_mock(query: str, location: str) -> List[dict]:
    ...
    print("[DATA] Using mock job data (add RAPIDAPI_KEY or SERPAPI_KEY to .env for real jobs)")
    results = []
    q_lower = query.lower()
...
def search_jobs(state) -> Dict:
    ...
    # Dynamic filters from Streamlit
    date_posted = state.get("date_posted", "month")
    work_modes = state.get("work_modes", ["Remote", "Hybrid", "On-site"])
    experience_level = state.get("experience_level", "Any")
    
    apply_limit = int(state.get("apply_limit", 10))
    applied_urls = {j.get("url") for j in state.get("applied", [])}

    print(f"\n[SEARCH] Searching: '{query}' in '{location}' (Date: {date_posted}, Modes: {work_modes}, Exp: {experience_level})")

    # Construct search query. Append remote/hybrid keyword to search query if narrow
    search_query = query
    if len(work_modes) == 1:
        search_query = f"{query} {work_modes[0]}"

    raw_jobs: List[dict] = []

    # 1. JSearch
    jsearch_jobs = search_jsearch(search_query, location, date_posted=date_posted)
    if jsearch_jobs:
        raw_jobs.extend(jsearch_jobs)
        print(f"[OK] JSearch returned {len(jsearch_jobs)} jobs")

    # 2. SerpAPI
    if len(raw_jobs) < 5:
        serp_jobs = search_serpapi(search_query, location, date_posted=date_posted)
        if serp_jobs:
            raw_jobs.extend(serp_jobs)
            print(f"[OK] SerpAPI returned {len(serp_jobs)} jobs")

    # 3. Mock fallback
    if not raw_jobs:
        raw_jobs = search_mock(query, location)

    # ── Normalize, filter, and score ─────────────────────────────────────────
    seen_urls = set()
    scored    = []
    ...
    for job in raw_jobs:
        ...
        is_remote_flag = job.get("source") == "jsearch" and job.get("job_is_remote", False)
        has_remote_text = any(k in title_desc for k in ["remote", "work from home", "wfh", "home-based", "telecommute"])
        has_hybrid_text = any(k in title_desc for k in ["hybrid", "flexible office", "work from anywhere", "flexible days"])
        
        if is_remote_flag or (has_remote_text and not has_hybrid_text):
            job_mode = "Remote"
        ...
        if job_mode not in work_modes:
            continue

        # --- Filter by experience level ---
        import re
        parsed_exp = job.get("parsed_experience_years")
        if parsed_exp is None:
            # Try to parse from description text
            exp_match = re.search(r"(\d+)\s*\+?\s*years?\s+(?:of\s+)?experience", title_desc)
            if exp_match:
                try:
                    parsed_exp = int(exp_match.group(1))
                except:
                    pass
        
        if parsed_exp is not None:
            job["experience_required"] = f"{parsed_exp}+ years"
        else:
            title_low = job.get('title', '').lower()
            if any(k in title_low for k in ["senior", "sr", "lead", "principal", "staff", "architect"]):
                job["experience_required"] = "Senior (3+ years)"
                parsed_exp = 4
            elif any(k in title_low for k in ["intern", "co-op", "trainee", "fresher", "junior", "jr"]):
                job["experience_required"] = "Entry Level / Fresher"
                parsed_exp = 0
            else:
                job["experience_required"] = "1-3 years (Mid Level)"
                parsed_exp = 2

        # Experience filters
        is_exp_match = True
        if experience_level == "Fresher/Entry Level (0-1 years)":
            if parsed_exp is not None and parsed_exp > 1:
                is_exp_match = False
        elif experience_level == "Mid Level (1-3 years)":
            if parsed_exp is not None and (parsed_exp < 1 or parsed_exp > 3):
                is_exp_match = False
        elif experience_level == "Senior Level (3-5 years)":
            if parsed_exp is not None and (parsed_exp < 3 or parsed_exp > 5):
                is_exp_match = False
        elif experience_level == "Lead/Director (5+ years)":
            if parsed_exp is not None and parsed_exp < 5:
                is_exp_match = False
    ...
    print(f"[OK] Total unique filtered jobs: {len(scored)}")
    for j in scored[:apply_limit]:
        has_jd = "[OK] JD" if len(j.get("description","")) > 100 else "[WARN] no JD"
        print(f"   * {j['score']:.2f} | {j['title']} @ {j['company']} [{j.get('work_mode')} | {j.get('experience_required')}]")
